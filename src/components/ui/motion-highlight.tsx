...
function MotionHighlight<T extends string>({ ...props }: MotionHighlightProps<T>) {
  ...
  // Note: this component manages an internal ref (`localRef`) but does not forward refs.
...
function MotionHighlightItem({
  ...
  // internal ref only; no forwarded ref handling
  ...
  if (asChild) {
    if (mode === 'children') {
        ...
        {
          key: childValue,
          ref: (localRef as unknown) as any,
