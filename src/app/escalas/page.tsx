...
export default function ScheduleViewer() {
    ...
    useEffect(() => {
        ...
        publicGetMethod<EscalaResumida[] | undefined>("v1/escala/resumed", setEscalasData);
    }, [escalasData])
                            ...
                            <Link href="/login" >
                                ...
                                <TooltipProvider>
                                    <Tooltip delayDuration={500}>
                                        <TooltipTrigger>
                                            <PanelLeftOpen className="md:h-6 md:w-6 sm:h-4 sm:w-4 hover:scale-110 hover:rotate-[360deg] transition-transform duration-500" />
                                        </TooltipTrigger>
                                        <TooltipContent className="bg-transparent border-none rounded-none shadow-none">Login</TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                            </Link>
                        </div>
                            ...
                            {isLoading ? (
                                ...
                                <Card key={escala.id} className={`${removeOverlay ? "animate-pulse" : ""} ${new Date(escala.data) < new Date() ? 'opacity-60 grayscale' : ''}`}>
                                    ...
                                    <CardFooter className="flex items-center justify-between">
                                        ...
                                        <div>
                                            <VerEscalaSomenteLeitura escalaId={escala.id} />
                                        </div>
                                    </CardFooter>
                                </Card>
                            ))}
